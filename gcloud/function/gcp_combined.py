...
def cache_report(hash_value, data):
    if not qs_bucket_name:
    ...
    try:
        ...
        print(f"{time.time()} cached report: {hash_value}")
    except Exception as e:
...
def handle_analyze(request):
    ...
    if not rerun and check_report_recent(md5):
        print(f"getting cached report for {md5}")
        report = get_report(md5)
        if report:
            resp = Response(report, mimetype='application/json')
            resp.headers.extend(get_cors_headers())
            return resp
        else:
            print(f"check_report_recent was true, but get_report failed for {md5}, proceeding to re-run analysis.")

    print(f"{time.time()} qs start for {md5}")
    try:
